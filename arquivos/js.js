				...
				calcShipping: function(t, i, r, o, a, s) {
					...
					n.ajax({
						...
						success: function(e) {
							...
							// s.find(".city-name").html(address.filterInvalidChar(e.city)),
							// s.find(".city-uf").html(address.filterInvalidChar(e.uf)),
							// o.find(".city").val(address.filterInvalidChar(e.city)),
							// o.find(".uf").val(address.filterInvalidChar(e.uf)),
							// a.find("input:first").focus(),
							// o.find("#neighborhood").val(address.filterInvalidChar(e.neighborhood)),
							// o.find("#neighborhood").trigger("input"),
							// o.find("#neighborhood").focus(),
							// o.find("#street").val(address.filterInvalidChar(e.street)),
							// e.street && (o.find("#number").focus(),
							// o.find("#street").trigger("input")),
							  ...
							  var xhr = new XMLHttpRequest();
								xhr.open ("GET", "https://cdn.apicep.com/file/apicep/"+cep+".json", true);
								...
								if(xhr.status == 200){
								  ...
								  a.removeClass("hide");
								  s.removeClass("hide");
								  address.noValidationCheckmark();
								  var dados = JSON.parse(xhr.responseText);

								   document.getElementById("city").value=dados.city;
								   document.getElementById("neighborhood").value=dados.district;
								   document.getElementById("street").value=dados.address;
								   uf=dados.state;
							...
							//  }
							  ...
							  // document.getElementById("zipcode").value;
							  // // d.hideItemLoader(r), r.closest(".form-group").addClass("group-error").find(".error-block").html(t.message), r.focus();
							  // // console.log("status2 = "+resp.status);
							  // }, 1500);
	...
	outbrain = function(e, t, n) {
		...
		return {
			...
			_dispatchPageView: function() {
				var t = "/checkout.php" === e.location.pathname ? "pageView_customer_step" : null;
		...
		return {
			...
			confirmOnExit: function() {
				e.onbeforeunload = function(t) {
					if ("/checkout.php" == e.location.pathname) return t = t || e.event, t && (t.returnValue = "Are you sure?"), "Are you sure?"
				}
			},
